...
/*double minScoreTriangulation(double dp[][50], vector<double>& A, int i = 0, int j = 0, double res = 0) {
  if (j == 0) j = A.size() - 1;
  if (dp[i][j] != 0) return dp[i][j];
  for (auto k = i + 1; k < j; ++k) {
    res = min(res == 0 ? INT_MAX : res, minScoreTriangulation(dp, A, i, k) + A[i] * A[k] * A[j] + minScoreTriangulation(dp, A, k, j));
  }
  return dp[i][j] = res;
}*/
